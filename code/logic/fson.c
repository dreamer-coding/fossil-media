...
fossil_media_fson_value_t *fossil_media_fson_parse(const char *json_text, fossil_media_fson_error_t *err_out) {
    ...
    if (*json_text == '{') {
        ...
        while (*json_text) {
            ...
            // Handle nested array with type: array<i32>, array<hex>, array<cstr>, array<mix>, array<array<i32>>, etc.
            else if (strncmp(base_type, "array", 5) == 0 && strchr(type, '<')) {
                // Parse array type: array<type>
                char elem_type[128] = {0};
                int is_mixed = 0;
                // Find matching angle brackets for nested types
                ...
                if (!lt) {
                    ...
                    if (err_out) {
                        ...
                        err_out->position = (size_t)(json_text - input_start);
                        snprintf(err_out->message, sizeof(err_out->message), "Malformed array type (no <)");
                    }
                    return NULL;
                }
                ...
                if (angle != 0) {
                    ...
                    if (err_out) {
                        err_out->code = FOSSIL_MEDIA_FSON_ERR_TYPE;
                        err_out->position = (size_t)(json_text - input_start);
                        snprintf(err_out->message, sizeof(err_out->message), "Malformed array type (unmatched < >)");
                    }
                    return NULL;
                }
                ...
                if (et_len == 0 || et_len >= sizeof(elem_type)) {
                    free(key);
                    free(type);
                    fossil_media_fson_free(obj);
                    if (err_out) {
                        err_out->code = FOSSIL_MEDIA_FSON_ERR_TYPE;
                        err_out->position = (size_t)(json_text - input_start);
                        snprintf(err_out->message, sizeof(err_out->message), "Malformed or empty array type");
                    }
                    return NULL;
                }
                memcpy(elem_type, et_start, et_len);
                elem_type[et_len] = '\0';
                if (strcmp(elem_type, "mix") == 0) {
                    is_mixed = 1;
                } else if (
                    strcmp(elem_type, "i8") != 0 &&
                    strcmp(elem_type, "i16") != 0 &&
                    strcmp(elem_type, "i32") != 0 &&
                    strcmp(elem_type, "i64") != 0 &&
                    strcmp(elem_type, "u8") != 0 &&
                    strcmp(elem_type, "u16") != 0 &&
                    strcmp(elem_type, "u32") != 0 &&
                    strcmp(elem_type, "u64") != 0 &&
                    strcmp(elem_type, "f32") != 0 &&
                    strcmp(elem_type, "f64") != 0 &&
                    strcmp(elem_type, "oct") != 0 &&
                    strcmp(elem_type, "hex") != 0 &&
                    strcmp(elem_type, "bin") != 0 &&
                    strcmp(elem_type, "char") != 0 &&
                    strcmp(elem_type, "cstr") != 0 &&
                    strcmp(elem_type, "bool") != 0 &&
                    strcmp(elem_type, "enum") != 0 &&
                    strcmp(elem_type, "datetime") != 0 &&
                    strcmp(elem_type, "duration") != 0 &&
                    strncmp(elem_type, "array<", 6) != 0 // allow nested
                ) {
                    // Unknown type
                    free(key);
                    free(type);
                    ...
                    if (err_out) {
                        err_out->code = FOSSIL_MEDIA_FSON_ERR_TYPE;
                        err_out->position = (size_t)(json_text - input_start);
                        snprintf(err_out->message, sizeof(err_out->message), "Unknown array element type: %s", elem_type);
                    }
                    return NULL;
                }
                // Move json_text to after the matching '>'
                json_text = gt;
                while (isspace((unsigned char)*json_text)) json_text++;
                ...
                if (*json_text == '[') {
                    ...
                    if (*json_text == ']') {
                        // Empty array
                        json_text++; // skip ']'
                        fossil_media_fson_value_t *empty_arr = fossil_media_fson_new_array();
                        if (!empty_arr) {
                            free(key);
                            free(type);
                            fossil_media_fson_free(obj);
                        ...
                        if (parsed_arr && fossil_media_fson_is_array(parsed_arr)) {
                            if (is_mixed) {
                                // array<mix>: allow any type, no conversion
                                val = parsed_arr;
                            } else if (strncmp(elem_type, "array<", 6) == 0) {
                                // Nested array: array<array<...>>
                                ...
                                for (size_t i = 0; i < parsed_arr->u.array.count; i++) {
                                    ...
                                    // Compose: x: array<...>: [ ... ]
                                    char *item_str = fossil_media_fson_stringify(item, 0, NULL);
                                    if (item_str) {
                                        char fake_fson[512];
                                        snprintf(fake_fson, sizeof(fake_fson), "x: array<%s>: %s", elem_type + 6, item_str);
                                        fossil_media_fson_value_t *converted = fossil_media_fson_parse(fake_fson, NULL);
                                        free(item_str);
                                        if (converted) {
                                            // Only take the value (since parse returns the array, not the object)
                                            if (converted->type == FSON_TYPE_ARRAY) {
                                                fossil_media_fson_array_append(typed_arr, converted);
                                            } else {
                                                fossil_media_fson_array_append(typed_arr, fossil_media_fson_clone(item));
                                                fossil_media_fson_free(converted);
                                            }
                                        } else {
                                            fossil_media_fson_array_append(typed_arr, fossil_media_fson_clone(item));
                                        }
                                ...
                                // Convert all elements to the requested type if possible
                                fossil_media_fson_value_t *typed_arr = fossil_media_fson_new_array();
                                if (!typed_arr) {
                                    fossil_media_fson_free(parsed_arr);
                                    free(key);
                                ...
                                for (size_t i = 0; i < parsed_arr->u.array.count; i++) {
                                    ...
                                    // Special case: array<hex> allows string hex values
                                    if (strcmp(elem_type, "hex") == 0) {
                                        if (item->type == FSON_TYPE_HEX) {
                                            converted = fossil_media_fson_clone(item);
                                        } else if (item->type == FSON_TYPE_CSTR && item->u.cstr) {
                                            char *endptr = NULL;
                                            uint64_t num = strtoull(item->u.cstr, &endptr, 16);
                                            if (endptr && *endptr == '\0') {
                                                converted = fossil_media_fson_new_hex(num);
                                            } else {
                                                // Try with 0x prefix
                                                char buf[64];
                                                snprintf(buf, sizeof(buf), "0x%s", item->u.cstr);
                                                num = strtoull(buf, &endptr, 16);
                                                if (endptr && *endptr == '\0') {
                                                    converted = fossil_media_fson_new_hex(num);
                                                }
                                            }
                                        } else if (item->type == FSON_TYPE_I32 || item->type == FSON_TYPE_I64) {
                                            uint64_t num = (item->type == FSON_TYPE_I32) ? (uint64_t)item->u.i32 : (uint64_t)item->u.i64;
                                            converted = fossil_media_fson_new_hex(num);
                                        }
                                    } else if (strcmp(elem_type, "i32") == 0) {
                                        if (item->type == FSON_TYPE_I32) {
                                            converted = fossil_media_fson_clone(item);
                                        } else if (item->type == FSON_TYPE_I64) {
                                            converted = fossil_media_fson_new_i32((int32_t)item->u.i64);
                                        } else if (item->type == FSON_TYPE_CSTR && item->u.cstr) {
                                            char *endptr = NULL;
                                            long v = strtol(item->u.cstr, &endptr, 10);
                                            if (endptr && *endptr == '\0') {
                                                converted = fossil_media_fson_new_i32((int32_t)v);
                                            }
                                        }
                                    } else if (strcmp(elem_type, "cstr") == 0) {
                                        if (item->type == FSON_TYPE_CSTR) {
                                            converted = fossil_media_fson_clone(item);
                                        } else if (item->type == FSON_TYPE_I32) {
                                            char buf[32];
                                            snprintf(buf, sizeof(buf), "%d", item->u.i32);
                                            converted = fossil_media_fson_new_string(buf);
                                        } else if (item->type == FSON_TYPE_I64) {
                                            char buf[32];
                                        ...
                                        if (item->type == FSON_TYPE_BOOL) {
                                            converted = fossil_media_fson_clone(item);
                                        } else if (item->type == FSON_TYPE_I32) {
                                            converted = fossil_media_fson_new_bool(item->u.i32 != 0);
                                        }
                                    } else if (strcmp(elem_type, "null") == 0) {
                                        if (item->type == FSON_TYPE_NULL) {
                                            converted = fossil_media_fson_clone(item);
                                        }
                                    } else {
